...
$st2 = $pdo->prepare('insert into city (id, uf_id, name, stat) values (?,'.
    '(select a1.id from uf a1 inner join country a2 on a1.country_id = a2.id where a2.iso = \'BR\' and a1.stat = left(cast(? as varchar), 2))'.
    ', ?, ?)');
$st3 = $pdo->prepare('update city set uf_id = COALESCE((select a1.id from uf a1 inner join country a2 on a1.country_id = a2.id where a2.iso = \'BR\' and a1.stat = left(cast(? as varchar), 2)), uf_id)'.
', name = ?, stat = ? where id = ?');

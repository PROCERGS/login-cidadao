...
class TagUri implements UriInterface
{
    ...
    public static function createFromString($string)
    {
        ...
        if (!preg_match("/{$overallRegex}/", $string, $m)) {
            throw new \InvalidArgumentException("The provided tag URI doesn't seem to be valid.");
        }

        $taggingEntity = self::getTaggingEntityRegex();
        if (preg_match("/^{$taggingEntity}/", $m['taggingEntity'], $m) !== 1) {
            throw new \InvalidArgumentException('Invalid taggingEntity: '.$m['taggingEntity']);
        }
